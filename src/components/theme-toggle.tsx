...
const ThemeToggle = () => {
  ...
  return (
    <button className="dark-mode-toggle w-16 h-8 rounded-full border-2 border-jet dark:border-floral flex items-center" onClick={toggleTheme}>
      {theme === 'dark' ? (
        <div className="w-full flex flex-row-reverse">
          <span className=""></span>
          <span className="pr-1" dangerouslySetInnerHTML={{ __html: SunIcon }} />
        </div>
      ) : (
        <div className="w-full flex flex-row justify-between">
          <span className="basis-1/2" dangerouslySetInnerHTML={{ __html: MoonIcon }} />
